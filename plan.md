...
**Generated:** 2026-05-29 from 13 architecture review files in `plans/`
...
**Status:** Already addressed in existing plan.md #1. Verify current value is at least `1 << 14` (16 MiB). If the Argon2 parameters changed, all existing encrypted data is undecryptable — a re-encryption migration may be needed.

**Action:** Verify the current memory cost value. If still 64 KiB, increase to `1 << 14` minimum. Add parameter versioning to `EncryptedPayload` if parameters changed.
...
**Files:** `snip-sync/src/main.rs` (CORS config block)

**Problem:** Warning message references `CORS_ALLOW_ALL=true` but the env var is never read. Users who set it get no effect.
...
**Files:** `snip-sync/src/main.rs` — `get_snippets`, `list_libraries` methods

**Problem:** `get_snippets` (line ~364) and `list_libraries` (line ~706) skip rate limiting. All other endpoints apply it.

**Fix:** Add `self.rate_limiter.allow()` check in both methods, matching the pattern used by mutating endpoints.
...
**Files:** `snip-sync/src/main.rs` — `register` method
...
**Fix:** Use IP address or a server-generated token as the rate limit key instead of client-provided `device_id`. The `RegisterRequest.device_id` field is ignored by the server anyway (server generates its own UUID).
...
**Files:** `snip-sync/src/main.rs`, `src/config.rs:61`

**Problem:** Client (`src/sync.rs`) configures TLS, but server default is `http://localhost:50051` (no TLS). The TLS handshake from the client will fail on default server config. Default server URL in client config is also HTTP.
...
### 2.1 — Sync Fall-Through Bug ✅ DONE (verify)

**Files:** `src/commands/sync_cmd.rs:224-251`

**Problem:** When `client.list_libraries()` fails at line 210, execution falls through (no `return`), causing sync to run twice or after a failure.

**Status:** Existing plan.md item. Verify fix is in place (should have `return Ok(?)` after Err branch at line 234).
...
**Files:** `src/sync.rs`, `src/sync_commands.rs`
...
**Fix:** Return `SnipError::runtime_error` if the filename doesn't exist in the library list.
...
**Fix:** Check if a `LibraryMeta` with the same `filename` already exists. If so, update its `library_id` instead of pushing a new entry. Match the pattern used by `add_existing_library` (line 362).
...
**Fix:** Return `Err(SnipError::toml_error(...))` on parse failure instead of returning defaults. Let callers decide how to handle (the backup is already created).
...
**Fix:** Extract into a tonic interceptor or helper function: `fn authenticate_and_rate_limit(&self, req: &impl HasApiKey) -> SnipResult<String>` returning `user_id`.
...
**Fix:** Track skipped snippet IDs during the sync loop and populate the response fields.
...
**Fix:** Add tie-breaking: `WHERE excluded.updated_at > snippets.updated_at OR (excluded.updated_at = snippets.updated_at AND excluded.device_id > snippets.device_id)`.
...
**Problem:**
- `verify_snippet_ownership` is unused (`#[allow(dead_code)]`)
- `DbError::Unauthorized` variant is never constructed
- `record_request` takes `_method` string but ignores it

**Fix:** Remove dead code. Remove `_method` parameter or use it in logging.
...
**Files:** `src/commands/run_cmd.rs`, `src/commands/clip_cmd.rs`, `src/commands/search_cmd.rs`, `src/commands/edit_cmd.rs`, `src/main.rs`

**Problem:** `run`, `clip`, `search`, `edit` accept `--config` but the parameter is `_config: Option<PathBuf>` and never used. Users think they can point to alternate config files but the flag does nothing.

**Fix:** Remove the `--config` flag from these 4 commands in `main.rs`. These commands use library mode and the config path is always resolved via `get_library_path`.

**Verification:** `cargo clippy --all-targets -- -D warnings` + `cargo test`.

---

### 4.2 — Dead Code in `run_cmd.rs`

**Files:** `src/commands/run_cmd.rs:36-40`

**Problem:** `Component::Normal(c)` branch checks `c.to_string_lossy().contains("..")` but `..` is always parsed as `Component::ParentDir`, never `Component::Normal`. Dead code.

**Fix:** Remove the `Component::Normal` branch entirely.

**Verification:** `cargo clippy --all-targets -- -D warnings` + `cargo test --lib`.

---

### 4.3 — Cron Interval Zero Validation

**Files:** `src/commands/cron_cmd.rs`, `src/main.rs:167`
...
**Files:** `src/commands/run_cmd.rs:102-111`
...
**Fix:** Move `Variable` struct to `src/utils/variables.rs` or a shared types module. Update imports in `ui.rs`.
...
These items were already tracked in the previous `plan.md`. Their status is preserved:
