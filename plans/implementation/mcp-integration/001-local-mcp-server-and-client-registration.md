> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `005-local-mcp-server-and-client-registration.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Local MCP Integration M001 — Local MCP server and client registration.
> Source roadmap: `plans/subsystems/mcp-integration-roadmap.md` (M001).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/mcp-integration/001-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-005-local-mcp-server-and-client-registration.md`.
