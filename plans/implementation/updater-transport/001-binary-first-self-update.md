> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `004-binary-first-self-update.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Updater Transport M001 — Binary-first self-update and restart integration.
> Source roadmap: `plans/subsystems/updater-transport-roadmap.md` (M001).
...
> Primary class: capability. Dependencies: interface release identity (exact tags).
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/updater-transport/001-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-004-binary-first-self-update.md`.
