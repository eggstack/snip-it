> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `017-eggfetch-0.2.0-updater-adoption-and-server-requalification.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Updater Transport M005 — eggfetch 0.2.0 updater adoption and server requalification.
> Source roadmap: `plans/subsystems/updater-transport-roadmap.md` (M005).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/updater-transport/005-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-017-eggfetch-0.2.0-updater-adoption-and-server-requalification.md`.
