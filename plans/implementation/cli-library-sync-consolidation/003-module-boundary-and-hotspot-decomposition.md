> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `010-module-boundary-and-hotspot-decomposition.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: CLI, Library, Selector, and Sync-Policy Consolidation M003 — Module boundary and hotspot decomposition.
> Source roadmap: `plans/subsystems/cli-library-sync-consolidation-roadmap.md` (M003).
...
> Primary class: infrastructure. Dependencies: hard M001, M002.
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/cli-library-sync-consolidation/003-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-010-module-boundary-and-hotspot-decomposition.md`.
