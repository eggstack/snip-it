> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `008-cli-surface-and-outcome-consolidation.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: CLI, Library, Selector, and Sync-Policy Consolidation M001 — CLI surface and outcome consolidation.
> Source roadmap: `plans/subsystems/cli-library-sync-consolidation-roadmap.md` (M001).
...
> Primary class: infrastructure. Dependencies: none (command contract baseline).
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/cli-library-sync-consolidation/001-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-008-cli-surface-and-outcome-consolidation.md`.
