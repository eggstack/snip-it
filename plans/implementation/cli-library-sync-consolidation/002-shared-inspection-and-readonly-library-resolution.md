> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `009-shared-inspection-and-readonly-library-resolution.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: CLI, Library, Selector, and Sync-Policy Consolidation M002 — Shared inspection and read-only library resolution.
> Source roadmap: `plans/subsystems/cli-library-sync-consolidation-roadmap.md` (M002).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/cli-library-sync-consolidation/002-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-009-shared-inspection-and-readonly-library-resolution.md`.
