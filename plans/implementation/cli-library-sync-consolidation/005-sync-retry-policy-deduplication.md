> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `012-sync-retry-policy-deduplication.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: CLI, Library, Selector, and Sync-Policy Consolidation M005 — Sync retry policy deduplication.
> Source roadmap: `plans/subsystems/cli-library-sync-consolidation-roadmap.md` (M005).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/cli-library-sync-consolidation/005-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-012-sync-retry-policy-deduplication.md`.
