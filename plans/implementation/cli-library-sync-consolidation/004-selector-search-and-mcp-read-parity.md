> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `011-selector-search-and-mcp-read-parity.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: CLI, Library, Selector, and Sync-Policy Consolidation M004 — Selector, search, and MCP read parity.
> Source roadmap: `plans/subsystems/cli-library-sync-consolidation-roadmap.md` (M004).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/cli-library-sync-consolidation/004-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-011-selector-search-and-mcp-read-parity.md`.
