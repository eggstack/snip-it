> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `018-eggserve-0.2.2-axum-runtime-adoption-trial.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Server Lifecycle and HTTP Runtime M002 — EggServe Axum-preserving runtime adoption trial.
> Source roadmap: `plans/subsystems/server-lifecycle-http-roadmap.md` (M002).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/server-lifecycle-http/002-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-018-eggserve-0.2.2-axum-runtime-adoption-trial.md`.
