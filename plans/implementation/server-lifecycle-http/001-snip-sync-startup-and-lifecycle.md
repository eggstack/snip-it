> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `003-snip-sync-startup-and-lifecycle.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Server Lifecycle and HTTP Runtime M001 — snip-sync startup and lifecycle management.
> Source roadmap: `plans/subsystems/server-lifecycle-http-roadmap.md` (M001).
...
> Primary class: capability. Dependencies: soft distribution artifact baseline.
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/server-lifecycle-http/001-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-003-snip-sync-startup-and-lifecycle.md`.
