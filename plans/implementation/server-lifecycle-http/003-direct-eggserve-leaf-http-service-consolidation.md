> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `019-direct-eggserve-leaf-http-service-consolidation.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Server Lifecycle and HTTP Runtime M003 — Direct EggServe leaf HTTP service consolidation.
> Source roadmap: `plans/subsystems/server-lifecycle-http-roadmap.md` (M003).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/server-lifecycle-http/003-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-019-direct-eggserve-leaf-http-service-consolidation.md`.
