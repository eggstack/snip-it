> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `020-eggserve-http-parity-and-closure-corrective.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Server Lifecycle and HTTP Runtime M004 — EggServe HTTP parity and closure corrective.
> Source roadmap: `plans/subsystems/server-lifecycle-http-roadmap.md` (M004).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/server-lifecycle-http/004-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-020-eggserve-http-parity-and-closure-corrective.md`.
