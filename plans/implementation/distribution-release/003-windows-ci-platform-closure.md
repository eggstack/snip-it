> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `006-windows-ci-platform-closure.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Distribution and Release M003 — Windows CI and platform closure.
> Source roadmap: `plans/subsystems/distribution-release-roadmap.md` (M003).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/distribution-release/003-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-006-windows-ci-platform-closure.md`.
