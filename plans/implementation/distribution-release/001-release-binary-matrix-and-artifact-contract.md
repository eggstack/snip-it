> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `001-release-binary-matrix-and-artifact-contract.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Distribution and Release M001 — Release binary matrix and artifact contract.
> Source roadmap: `plans/subsystems/distribution-release-roadmap.md` (M001).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/distribution-release/001-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-001-release-binary-matrix-and-artifact-contract.md`.
