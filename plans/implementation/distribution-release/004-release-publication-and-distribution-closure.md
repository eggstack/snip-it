> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `007-release-publication-and-distribution-closure.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Distribution and Release M004 — Release publication and distribution closure.
> Source roadmap: `plans/subsystems/distribution-release-roadmap.md` (M004).
...
> Primary class: capability. Dependencies: hard M003 (transitively M001, M002).
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/distribution-release/004-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-007-release-publication-and-distribution-closure.md`.
...
- `bash scripts/release-check.sh verify` passed after the crate publication, including release-profile builds, production installer seams, package checks, and the self-update/lifecycle checks. Focused updater contract tests and the stopped/running lifecycle decision table also passed. The older standalone `snip-sync` 0.1.4 binary predates binary-first updating and selected its documented Cargo fallback; the current updater seam was therefore used for deterministic contract validation, as permitted by this plan. A local HTTP seam run with `--features test-helpers` selected `snip-sync-v0.1.6` and the ARM64 asset without changing files or processes.
- The default public Unix bootstrap consumed the published ARM64 asset on the local `aarch64` Linux host without Cargo fallback. The native [consumer smoke run 33988462739](https://github.com/eggstack/snip-it/actions/runs/33988462739) independently passed the public `x86_64-unknown-linux-gnu` and `aarch64-unknown-linux-gnu` Unix installers plus the Windows x86_64 PowerShell installer, with version identity checks and no Cargo fallback.
