> **Planning-convention migration note.** This file was promoted via `git mv` from pre-convention flat plan `002-bootstrap-installers.md` (history preserved). Its body is unchanged historical evidence.
> New-convention identity: Distribution and Release M002 — Binary-first bootstrap installers.
> Source roadmap: `plans/subsystems/distribution-release-roadmap.md` (M002).
...
> Status vocabulary mapping: pre-convention `Status: complete` means `closed` with closure record at `plans/closure/distribution-release/002-status.md`.
> Pre-convention archive pointer: `plans/archive/flat-002-bootstrap-installers.md`.
