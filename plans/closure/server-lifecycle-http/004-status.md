# EggServe HTTP parity and closure corrective — Closure Status
...
- `plans/archive/flat-020-eggserve-http-parity-and-closure-corrective.md`
...
Primary class: polish. Dependencies: hard M002, M003.

## 1. Executive finding

M004 is closed. The corrective restored the historical CORS `Vary` semantics (`Vary: origin` on non-allow-all, omitted for allow-all), locked the empty router 404/405 representation, known-route preflight `Allow`, and the preflight/ordinary security-header boundary with real-socket tests, and completed the M002/M003 evidence record. C remains final; no framework reintroduction.

## 2. Requirement-to-evidence matrix

| Requirement | Evidence | Result | Notes |
|---|---|---|---|
| CORS Vary contract restored | Implementation file + `tests/snip_sync_lifetime.rs` | pass | Proven contract reproduced, not a new header value. |
| 404/405 + preflight Allow + header boundary socket-proven | Implementation file + socket tests | pass | Real-socket assertions. |
| M002/M003 evidence completed | Implementation file | pass | Commit `10aeb994`, run `36011491151`. |
...
Recorded verification: implementation commit `10aeb994` plus follow-up parity commit; hosted run `36011491151`; local `scripts/check.sh` and production-seam checks. No unresolved medium-or-higher finding remains.
