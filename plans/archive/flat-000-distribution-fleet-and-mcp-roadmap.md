> **Archived canonical predecessor.** Pre-convention `plans/000-distribution-fleet-and-mcp-roadmap.md` moved here via `git mv` (history preserved; body unchanged below this note).
> It is superseded as direction by `plans/000-long-term-specification.md`, `plans/001-terminology-and-domain-model.md`, `plans/002-long-term-roadmap.md`, and the subsystem roadmaps `distribution-release` (distribution half) and `mcp-integration` (MCP half).
> Status: archived.
