...
- **`auto-sync-worker`** — hidden subcommand (clap `hide = true`) that debounces, invokes `crate::sync_commands::run_sync` directly, conditionally clears the observed generation, records status, and exits.
...
snp auto-sync-worker (detached helper process)
  ...
  → execute_sync(state_dir, policy)
    → run canonical sync directly
      → network/request retry bounds remain in sync client
      → exact-generation clear and status update
...
- Parent never acquires the worker execution lock — only the detached helper does. The helper invokes `run_sync` directly.
