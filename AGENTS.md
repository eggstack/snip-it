...
- `WorkerLock` RAII (`src/auto_sync/lock.rs`): atomic acquisition via `OpenOptions::create_new(true)`; `WorkerLockContents { pid, started_at_unix_ms, nonce }`; stale detection via `kill -0 pid` + 5-minute age threshold; 0o600 permissions on Unix. **Release 5 corrective:** the parent never acquires the lock — it is the worker's responsibility.
- `PendingState` schema v2 (`src/auto_sync/pending.rs`): monotonic `generation`, `created_at_unix_ms`, `snapshot` (Mutation/Nil), CRC32 `integrity` field. Conditional `clear_if_generation_matches(observed_generation)` prevents stale workers from clobbering fresh state. v1 markers migrate transparently on load.
- **Release 5 corrective API split:** `pending::record_pending_mutation` is the only writer/incrementer of the pending generation. `worker::schedule_existing_pending` reads the marker but never mutates it. `mark_pending` is retained as a legacy alias. The parent path is now strictly `record → schedule` with no opportunity for a stale-worker-clears-newer-state race.
...
- Auto-sync tests (`src/auto_sync/`): unit tests in each submodule cover policy resolution (`policy.rs`), PendingState schema v2 roundtrip, generation monotonicity, conditional clear, CRC32 integrity, v1→v2 migration, atomic write, 0o600 permissions, no-secrets guarantee (`pending.rs`), WorkerLock RAII, atomic acquire, stale detection (kill -0 / 5-min age), nonce uniqueness, lock file permissions (`lock.rs`), `spawn_worker` plumbing and detach flags (`spawn.rs`), `WorkerOutcome` mapping, `SpawnResult` matrix, `execute_sync`, `startup_recover`, `run_locked` debounce and follow-up cycle, `run_async_with_timeout` (`worker.rs`), and `notify_local_mutation` for disabled policy, sync-merge suppression, and result variants (`notification.rs`).
