...
- `snp update` uses in-process `eggfetch-core =0.1.7` (`standard-http1`+`redirects`+`tls-rustls`+`tls-native-roots` only; strict eggfetch redirects + native `Timeout.total`, snip-it keeps the initial-HTTPS guard); `snip-sync update` keeps external `curl` (embedding TLS grew the server +41% past the 10% gate). `tests/architecture.rs` pins the no-`curl`/lean-profile/delegated-timeout properties; transport tests live in `src/update.rs` (`test-support`) with a std-only loopback fixture.
