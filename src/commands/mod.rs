...
"#,
    ...
    #[test]
    fn test_run_explicit_sync_exists_and_has_correct_signature() {
        ...
        // run_explicit_sync should compile and be callable.
        // It will fail because no sync server is running, but the
        // important thing is that it compiles with the right signature.
        let result = super::run_explicit_sync(&TEST_RT);
        assert!(result.is_err(), "should fail without a sync server");
    }
}
