//! **Layer: Application**
//!
//! Backup archive primitives: manifest types, path validation, snapshot
//! reading, and atomic staging. Independently testable with no CLI
//! orchestration; [`run`](super::backup_cmd::run) in `backup_cmd` owns
//! lock acquisition, generation guards, and output reporting.
...
pub(crate) fn read_for_snapshot(path: &Path, label: &str) -> SnipResult<Vec<u8>> {
...
pub(crate) fn read_library_generation(config_dir: &Path) -> SnipResult<u64> {
...
pub(crate) fn validate_canonical_containment(
    path: &Path,
    root: &Path,
    label: &str,
) -> SnipResult<PathBuf> {
...
pub(crate) fn validate_toml_content(bytes: &[u8], label: &str) -> SnipResult<()> {
...
pub(crate) fn atomic_write_backup(
    staging_dir: &Path,
    final_dir: &Path,
    files: &[(&str, &str, &[u8])],
...
pub(crate) fn redact_sync_config(content: &str) -> SnipResult<String> {
